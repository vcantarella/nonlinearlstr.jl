...
using NLPModels:
    bound_constrained, finalize, AbstractNLSModel, residual, jac_residual, obj, grad
using Pkg, Revise
...
for prob_name in probs
    ...
    try
        ...
        push!(
            ...
            (
                problem = prob_data.name,
                solver = "active_set_svd_trust_region",
                converged = converged,
                final_obj = final_obj,
                iterations = iterations,
            ),
    ...
    try
        ...
        push!(
            ...
            (
                problem = prob_data.name,
                solver = "active_set_svd_trust_region_scaled",
                converged = converged,
                final_obj = final_obj,
                iterations = iterations,
            ),
    ...
    try
        result_scipy = scipy.optimize.least_squares(
            prob_data.residual_func,
            prob_data.x0,
            jac = prob_data.jacobian_func,
            bounds = (prob_data.bl, prob_data.bu),
        ...
        push!(
            ...
            (
                ...
                converged = converged,
                final_obj = final_obj,
                iterations = pyconvert(Int, result_scipy.nfev),
            ),
        )
    catch e
...
summary_table = @chain df_perf begin
    ...
    @mutate(success_rate = round(100 * n_converged / n_problems, digits = 2))
        ...
        if nrow(failed_problems) == 0
            ...
            for (i, fail_row) in enumerate(eachrow(failed_problems))
                if isfinite(fail_row.ratio)
                    println(
                        "    $(i). $(fail_row.problem): final_obj=$(round(fail_row.final_obj, digits=6)), " *
...
problem_difficulty.failure_rate = round.(
    100 * problem_difficulty.n_solvers_failed ./ problem_difficulty.n_total_solvers,
    digits = 1,
)
...
if nrow(problem_difficulty) > 0
    ...
    for (i, prob_row) in enumerate(eachrow(problem_difficulty))
        println(
            "  $(i). $(prob_row.problem): $(prob_row.n_solvers_failed)/$(prob_row.n_total_solvers) solvers failed " *
