...
function compare_with_best(df::DataFrame)
    ...
    min_solutions = combine(groupby(df_proc, :problem), :final_cost => minimum => :min_solution)
    ...
    df_proc.final_close_abs = abs.(df_proc.final_cost .- df_proc.min_solution) ./ abs.(df_proc.min_solution) .< 1e-4
...
function build_performance_plots(df_proc::DataFrame)
    ...
    for solver_n in solvers
        ...
        lines!(ax1, df_solver.time, df_solver.cumulative_success, label=solver_n, linewidth=2)
    end
    ...
    barplot!(ax2, 1:n_solvers, summary_df.percentage_success .* 100, color =:steelblue,
    alpha=0.7)
